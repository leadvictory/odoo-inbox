...
class Partner(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        args = [[1, '=', 1] if (arg[0] == 'create_uid' and arg[1] == '=' and arg[2] == self.env.user.id) else arg for arg in args]
        if not self.env.user.has_group('base.group_system'):
            contact_books = self.env['res.contact.book'].search(['|', ('user_ids', 'in', self.env.user.id), ('isPublic', '=', True)])
            args += ['|', ('create_uid', '=', self.env.user.id), ('contact_book_id', 'in', contact_books.ids)]
        # _logger.info(f"search args are {str(args)})")
        return super().search(args, offset, limit, order, count)

    @api.model
    def search_global(self, args, offset=0, limit=None, order=None, count=False):
        args = [[1, '=', 1] if (arg[0] == 'create_uid' and arg[1] == '=' and arg[2] == self.env.user.id) else arg for arg in args]
        return super().search(args, offset, limit, order, count)
