    ...
    value: function attachDragAndDropEvent() {
      ...
      this.$dropzone.on('drop', function (event) {
        ...
        if (dataTransfer && dataTransfer.files && dataTransfer.files.length) {
          ...
          const files_input = $("input[name='compose_attachments']")[0]; // Get the DOM element
          const _dataTransfer = new DataTransfer(); // Create a DataTransfer object
          // Add existing files to the DataTransfer object
          if (files_input.files) {
            for (let i = 0; i < files_input.files.length; i++) {
              _dataTransfer.items.add(files_input.files[i]);
            }
          }

          // Add new files from the drag and drop event to the DataTransfer object, with type checking
          for (let i = 0; i < dataTransfer.files.length; i++) {
            const file = dataTransfer.files[i];
            if (file && !(file.type.startsWith('image/') || file.type.startsWith('text/'))) {
              _dataTransfer.items.add(file); // Only add if it's an image or text file
            }
          }

          files_input.files = _dataTransfer.files;
          const event = new Event('change', { bubbles: true });
          files_input.dispatchEvent(event);
        } else {
