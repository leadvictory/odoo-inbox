        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                for email in email_list:
                    ...
                    try:
                        match = re.search(r'<(.*?)>', mail.email_from)
                        try:
                            from_email = match.group(1)
                            servers = IrMailServer.search([('smtp_user', '=', from_email)])
                            server = servers[0]
                            _smtp_session = self.env['ir.mail_server'].connect(mail_server_id=servers[0].id, smtp_from=from_email)
                            fetchmail_server = self.env['fetchmail.server'].search([
                                ('user', '=', from_email),
                                ('server_type', '=', 'imap')
                            ])
                            imap_server = fetchmail_server[0].connect()
                            imap_server.select('INBOX.Sent')
                            imap_server.append('"INBOX.Sent"', '\\Seen', imaplib.Time2Internaldate(time.time()), msg.as_bytes())
                        except:
                            server = mail.mail_server_id
                            _smtp_session = self.env['ir.mail_server'].connect(mail_server_id=server, smtp_from=mail.email_from)
                        res = IrMailServer.send_email(
                            msg, mail_server_id=server.id, smtp_session=_smtp_session)
                        if processing_pid:
